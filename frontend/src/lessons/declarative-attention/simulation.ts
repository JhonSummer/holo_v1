...
export function selectedChunkIds(state: MemoryState, config: MemoryConfig): number[] {
  ...
  return config.chunks.filter(chunk => state.focusedChunks.includes(chunk.id)).map(chunk => chunk.id)
}
...
export function nextRead(state: MemoryState, config: MemoryConfig, text = 'next token'): ReadEvent {
  ...
  const readTokens = alwaysRead + config.chunks.filter(chunk => chunkIds.includes(chunk.id))
    .reduce((total, chunk) => total + chunk.tokens, 0)
  const fraction = readTokens / fullTokens
  return {
    id: state.events.length + 1, mode: state.mode, chunkIds,
    responseTokens: state.responseTokens, readTokens, fullTokens,
    readBytes: readTokens * bytesPerToken(config), fullBytes: fullTokens * bytesPerToken(config),
    fraction, durationMs: Math.round(550 + 1850 * fraction), text,
  }
}

export function residentBytes(state: MemoryState, config: MemoryConfig): number {
  if (!stageReached(state, 'prefill')) return 0
  const tokens = config.scaffold_tokens + state.responseTokens + config.chunks.reduce((n, c) => n + c.tokens, 0)
  return tokens * bytesPerToken(config)
}

/** No mutable global state: reset, replay and tests all use the same reducer. */
export function applyCommand(state: MemoryState, command: DACommand, config: MemoryConfig): MemoryState {
  switch (command.op) {
    case 'daStage':
    ...
    case 'daMode': {
      ...
      const chunks = [...new Set(command.args.chunks ?? state.focusedChunks)]
        .filter(id => config.chunks.some(chunk => chunk.id === id))
    ...
    case 'daDecode': {
      ...
      return { ...state, stage: 'decode', responseTokens: state.responseTokens + 1, events: [...state.events, event] }
    }
  }
}
