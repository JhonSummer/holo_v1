...
export function playAudio(element: HTMLAudioElement, url: string, text: string, signal: AbortSignal): Promise<boolean> {
...
export function browserSpeak(text: string, signal: AbortSignal, voice: SpeechSynthesisVoice | null): Promise<boolean> {
  ...
  return new Promise(resolve => {
    ...
    const cancel = () => { finish(); window.speechSynthesis.cancel() }
    const timeout = setTimeout(cancel,speechCapMs(text))
    signal.addEventListener('abort',cancel,{once:true})
...
export function browserListen(setRecording: (value:boolean)=>void, ref: {current:any}, signal:AbortSignal): Promise<string> {
  ...
  return new Promise(resolve => {
    ...
    try { recognition.start(); setRecording(true) } catch { finish() }
  })
}
