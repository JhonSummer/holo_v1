...
class SpatialBoundary extends Component<{children:ReactNode;fallback:ReactNode}, {failed:boolean}> {
  state = { failed:false }
...
export default function DeclarativeAttentionLesson({ pod, catalogue, onSelect, onSignOut }: {
  ...
  const [comparison, setComparison] = useState<{mode:string;fraction:number;resident:number} | null>(null)
  ...
  const interrupt = () => { request.current?.abort(); setThinking(false); lesson.stop(); voice.cancelListening() }
  ...
  const ask = async (text: string) => {
    ...
    setThinking(true)
    try {
      const response = await askTutor(pod.id, text.trim(), lesson.current.current, controller.signal, historyRef.current, pod.scene.params)
      if (controller.signal.aborted) return
      setThinking(false)
      historyRef.current = [...historyRef.current, {role:'user',content:text.trim()}, {role:'assistant',content:response.text}].slice(-12) as ConversationTurn[]
  ...
  const sceneProps = {state:lesson.state,config:pod.scene.params,cue:lesson.visual,declaration:lesson.declaration,prefilledSlots:lesson.prefilledSlots,onChunk:toggleChunk}
  ...
  return <main className="da-lesson">
    ...
    <section className="da-workspace">
      <div className="da-diagram-panel">
        ...
        <div className="da-view-controls"><button aria-pressed={view==='spatial'} onClick={()=>setView('spatial')}>Spatial view</button><button aria-pressed={view==='diagram'} onClick={()=>setView('diagram')}>Diagram · low power</button><span>{lesson.declaration ? `Output declares ${lesson.declaration}` : lesson.visual ? `${lesson.visual.kind.toUpperCase()} phase` : 'Explore the GPU'}</span></div>
        {view==='diagram' ? <GpuStage {...sceneProps} /> : <SpatialBoundary fallback={<><p>Spatial view unavailable. The diagram and all controls still work.</p><GpuStage {...sceneProps} /></>}><Suspense fallback={<div className="da-loading">Opening the spatial GPU…</div>}><GpuSpatial {...sceneProps} reducedMotion={reducedMotion} /></Suspense></SpatialBoundary>}
        <div className="da-legend"><span style={{ color: '#ffb35c' }}>● weights</span><span style={{ color: '#aeb8cb' }}>● scaffold</span><span style={{ color: '#e98772' }}>● context KV</span><span style={{ color: '#5dd39e' }}>● response KV</span><span>Packets = payload (weights, inputs or KV) · density & time illustrative</span></div>
      </div>
    ...
    <section className="da-experiment" aria-label="Attention experiment">
      ...
      <button className="da-primary" disabled={!ready || !!lesson.visual || !!lesson.declaration || lesson.state.events.length >= 128}
      ...
      {lesson.state.events.length>=128 && <span className="da-hint">128-step experiment complete. Replay Prefill to start a fresh comparison.</span>}
      <button disabled={!ready} onClick={()=>setComparison({mode:lesson.state.mode,fraction:currentRead.fraction,resident:residentBytes(lesson.state,pod.scene.params)})}>Save comparison</button>
      <button onClick={()=>{interrupt();setPractice('residency')}}>Check my understanding</button>
    </section>
    {comparison && <p className="da-comparison" role="status">Saved {comparison.mode}: {(comparison.fraction*100).toFixed(1)}% read → now {(currentRead.fraction*100).toFixed(1)}%. Resident KV {comparison.resident===residentBytes(lesson.state,pod.scene.params)?'unchanged':'grew with the reply'}. <button onClick={()=>setComparison(null)}>Clear comparison</button></p>}
    ...
    <section className="da-narrator">
      ...
      {history.length>0 && <details className="da-conversation"><summary>Conversation · {history.length/2} recent exchanges</summary>{history.map((turn,i)=><p key={i}><strong>{turn.role==='user'?'You':'Tutor'}:</strong> {turn.content}</p>)}</details>}
    </section>
