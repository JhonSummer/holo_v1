...
export default function useLessonPlayback(pod: DeclarativeAttentionPod, speak: (text: string) => Promise<boolean>, stopSpeaking: () => void) {
  const playback = useMemo(() => new LessonPlayback(pod, speak, stopSpeaking), [pod, speak, stopSpeaking])
  ...
  return { ...snapshot, current, stop: playback.stop, setCaption: playback.setCaption,
    play: () => playback.play(), step: playback.step, restart: playback.restart, replay: playback.replay,
    goTo: playback.goTo, explore: playback.explore, resolveCheckpoint: playback.resolveCheckpoint }
}
