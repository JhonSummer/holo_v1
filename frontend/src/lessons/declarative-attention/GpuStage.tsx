...
export default function GpuStage({ state, config, cue, declaration, prefilledSlots, onChunk }: {
  state: MemoryState; config: MemoryConfig; cue: VisualCue | null; declaration: string | null; prefilledSlots: number; onChunk: (id: number) => void
}) {
  ...
  const tag = declaration ?? (state.mode === 'focus' ? `<focus magic_chunks="${state.focusedChunks.join(',')}">` : `<${state.mode}>`)
  const responseCount = Math.max(0, state.responseTokens - (cue?.kind === 'read' || cue?.kind === 'response' ? 1 : 0))
  return <svg className={`da-stage ${cue?.kind === 'residency' ? 'da-residency-pulse' : ''}`} viewBox="0 0 1000 470" role="group" aria-label="GPU memory diagram. Masked chunks stay resident; packets show reads.">
    ...
    {weightNames.map((name, i) => <g key={name}>
      <rect x={64 + i * 135} y="296" width="125" height="25" rx="4" fill={weights && (cue?.kind !== 'weights' || i < (cue.slot ?? 0)) ? COLORS.weights : 'transparent'} stroke={COLORS.weights} />
      <text x={126 + i * 135} y="313" textAnchor="middle" className="da-svg-tiny" fill={weights ? '#05060a' : COLORS.weights}>{name}</text>
    </g>)}
    {[{ id: 0, label: 'SYS', tokens: config.scaffold_tokens, color: '#aeb8cb' }, ...config.chunks].map((chunk, i) => {
      ...
      return <g key={chunk.id} className={chunk.id && resident ? 'da-chunk' : ''}
        ...
        <rect x={slotX(i, config.chunks.length)} y="337" width={slotWidth(config.chunks.length) - 8} height="54" rx="6" fill={filled ? chunk.color : 'transparent'}
          fillOpacity={active ? .28 : .045} stroke={resident ? chunk.color : '#60566a'} strokeOpacity={active ? 1 : .35} />
        <text x={slotCenter(i, config.chunks.length)} y="358" textAnchor="middle" fill={chunk.color} className="da-svg-label" opacity={filled ? 1 : .4}>{chunk.label}</text>
        <text x={slotCenter(i, config.chunks.length)} y="378" textAnchor="middle" className="da-svg-tiny">{filled ? `${chunk.tokens.toLocaleString()} tok` : 'empty'}</text>
        {filled && <text x={slotCenter(i, config.chunks.length)} y="403" textAnchor="middle" className="da-svg-tiny" fill={active ? '#badfce' : '#b8a4b4'}>{active ? 'READ' : 'MASKED'}</text>}
      </g>
    })}
    <text x="70" y="420" className="da-svg-tiny" fill={COLORS.hbm}>HBM · fixed KV seats</text>
    {Array.from({length: Math.min(12, responseCount)}, (_, i) => <rect key={i} x={248 + i * 13} y="410" width="10" height="11" fill={COLORS.response} />)}
    <text x="590" y="420" textAnchor="end" className="da-svg-tiny" fill={COLORS.response}>reply: {responseCount} tok</text>
    ...
    {!stageReached(state, 'request') && <g opacity={weights ? .4 : .85}>
      {weightNames.map((name, i) => <g key={name}><rect x="738" y={150+i*38} width="216" height="29" rx="4" fill="#49352a" stroke={COLORS.weights} /><text x="846" y={170+i*38} textAnchor="middle" className="da-svg-label" fill={COLORS.weights}>{name}</text></g>)}
    </g>}
    {stageReached(state, 'request') && !output && [
      { label: 'SYS + question', color: '#aeb8cb' }, ...config.chunks,
    ].map((chunk, i) => <g key={i} opacity={resident ? .4 : 1}>
      <rect x="738" y={143 + i * 27} width="216" height="23" rx="4" fill={chunk.color} fillOpacity=".16" stroke={chunk.color} />
      <text x="750" y={160 + i * 27} className="da-svg-label" fill={chunk.color}>{i ? `magic chunk ${chunk.label}` : chunk.label}</text>
    </g>)}
    {output && <g>
      <rect x="733" y="136" width="227" height="35" rx="5" fill="#243341" />
      <text x="743" y="158" textLength={tag.length > 32 ? 207 : undefined} lengthAdjust="spacingAndGlyphs" className="da-svg-code" fill="#f0d58c">{tag}</text>
      {state.events.slice(0, responseCount).slice(-6).map((item, i) => <text key={item.id} x="742" y={207 + i * 30} className="da-svg-label" fill={item.mode === 'global' ? '#b5c4d3' : '#88d7ad'} textLength={item.text.length > 26 ? 205 : undefined} lengthAdjust="spacingAndGlyphs">{item.text}</text>)}
