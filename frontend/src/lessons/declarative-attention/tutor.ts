...
export function validateCommands(value: unknown, config: MemoryConfig): DACommand[] {
  ...
  for (const command of value) {
    if (!command || typeof command !== 'object' || !command.args || typeof command.args !== 'object') throw new Error('Invalid tutor command.')
    const {op,args} = command
    if (op === 'daStage' && ['empty','weights','request','prefill','decode'].includes(args.stage)) continue
    if (op === 'daDecode' && typeof args.text === 'string' && args.text.length <= 80) continue
    if (op === 'daMode' && ['global','focus','local'].includes(args.mode) && Array.isArray(args.chunks)
      && args.chunks.length <= 8 && args.chunks.every((id:unknown) => typeof id === 'number' && validIds.has(id))
      && (args.mode !== 'focus' || args.chunks.length > 0)) continue
...
export function askTutor(podId: string, query: string, scene: MemoryState, signal: AbortSignal, history: ConversationTurn[], config: MemoryConfig): Promise<{ text: string; commands: DACommand[] }> {
  return new Promise((resolve, reject) => {
    ...
    ws.onopen = () => { if (!settled) ws.send(JSON.stringify({ query, scene: { ...scene, events: scene.events.slice(-8),
      computed: {nextRead:nextRead(scene,config),residentBytes:residentBytes(scene,config)}, conversation: history.slice(-12) } })) }
    ...
    ws.onmessage = event => {
      try {
        ...
        if (message.type === 'narration') {
          if (typeof message.text !== 'string' || text.length+message.text.length>1200) throw new Error('Invalid narration')
          text += message.text
        }
